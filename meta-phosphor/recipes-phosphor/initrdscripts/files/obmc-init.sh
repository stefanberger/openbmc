...
# If securityfs is available and 'no-ima' is not found in $optfile then
...
activate_ima_evm() {
	...
	then
		...
		then
			...
			# Initramfs must run in tmpfs: rootfstype=tmpfs
			if grep -q \
				-E "appraise .*func=(MMAP_CHECK|BPRM_CHECK)" \
				"$ima_policy"
			then
				write_file_signatures /ima_file_signatures ima
				write_file_signatures /evm_file_signatures evm
			fi
			if ! echo "$ima_policy" > \
