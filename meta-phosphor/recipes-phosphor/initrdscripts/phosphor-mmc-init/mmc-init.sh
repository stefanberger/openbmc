...
activate_ima_evm() {
	...
	# If not running on QEMU: if secure boot is disabled then also disable
	# IMA & EVM
	if test -z "$("$rodir/usr/bin/dmesg" | grep -i qemu)" -a \
	        -n "$("$rodir/usr/bin/gpioget" gpiochip0 70 | grep 0)"
	then
		return
	fi
	...
	then
		...
		then
			...
			then
				...
				# Use the signed version of busybox and other tools
				# found in $rodir and used later on in this script
				for fn in \
					/bin/busybox.nosuid \
					/sbin/fsck.ext4 \
					/usr/sbin/mkfs.ext4 \
