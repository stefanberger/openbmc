...
also not be load. To use this option one should inspect the firmware
...
Other options for disabling IMA and EVM individually are well-known Linux
kernel command line options such as ``ima_appraise=off|log|fix|enforce`` or
``evm=fix``.
