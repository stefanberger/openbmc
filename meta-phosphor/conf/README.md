...
Creating your own CA and signing key
====================================

When using IMA and EVM in a production system, it is strongly recommended that
dedicated keys be used rather than the default debug/testing keys. Example
scripts are provided to create first a self-signed CA key and then a NIST-P256
file signing key along with an X509 certificate. The scripts can be run as
follows to first create the self-signed CA and save its key into a directory
symbolically referred to as ``/production``:

    cd meta-security/meta-integrity/scripts
    ./ima-gen-local-ca.sh
    cp ima-local-ca.pem /production

Now generate the IMA signing key and its certificate:

    ./ima-gen-CA-signed.sh
    cp x509_ima.der privkey_ima.pem

It is important that the resulting keys pass the following test:

    openssl verify -CAfile /production/ima-local-ca.pem \
         /production/x509_ima.der
    /production/x509_ima.der: OK

OpenBMC can now be built with the following entries in ``conf/local.conf``:
...
There, RSA keys can be generated with the following scripts:
...
In case a signed file must be re-signed, for example after modification,
it is necessary to first get rid of the signatures in security.ima and
security.evm extended attributes since the system's EVM configuration
prevents modification of file metadata. The following shows that
re-signing a file with existing signatures fails:

    root@p10bmc:~# mount -o remount,rw /
    root@p10bmc:~# getfattr -m ^sec -e hex --dump /usr/bin/xzmore
    getfattr: Removing leading '/' from absolute path names
    # file: usr/bin/xzmore
    security.evm=0x0502046730eefd00473045022100ba0f67cfa...
    security.ima=0x0302046730eefd00483046022100d8be87117...

    root@p10bmc:~# evmctl sign --imasig -a sha256 --portable --m32 \
        --key /tmp/privkey_ima.pem /usr/bin/xzmore
    ...
    Setting IMA sig xattr failed: /usr/bin/xzmore (errno: Operation not permitted)

The following sequence can be used to re-sign the file:

    root@p10bmc:~# cp /usr/bin/xzmore /usr/bin/xzmore.bak
    root@p10bmc:~# mv /usr/bin/xzmore.bak /usr/bin/xzmore
    root@p10bmc:~# evmctl sign --imasig -a sha256 --portable --m32 \
        --key privkey_ima.pem /usr/bin/xzmore
